        ...
        public async Task<IActionResult> SearchAccounts([FromQuery] string query)
        {
            try
            {
                ...
                var queryable = from ct in _context.CustomerTransactions
                                join ca in _context.CustomerAccounts on ct.CustomerAccountId equals ca.CustomerAccountId into caGroup
                                from ca in caGroup.DefaultIfEmpty()
                                join ja in _context.JointAccounts on ca.CustomerAccountId equals ja.CustomerAccountId into jaGroup
                                from ja in jaGroup.DefaultIfEmpty()
                                join c in _context.Customers on ja.CustomerId equals c.CustomerId into cGroup
    ...
    [FromQuery] string? transactionType = null, [FromQuery] bool latestOnly = false)
{
    ...
    // Clean up culture fallback safely inside the scope
    var PhCulture = new System.Globalization.CultureInfo("en-PH");

    var document = Document.Create(container =>
    {
        // This is the ONLY place .Page() should appear, and it requires a lambda argument!
        container.Page(page =>
        {
            ...
            // Footer definition is directly attached to the page element
            page.Footer()
                .AlignRight()
                .Text(x =>
                {
    ...
    return File(pdfBytes, "application/pdf", $"TransactionReport_{DateTime.Now:yyyyMMddHHmmss}.pdf");
}
    ...
    string? transactionType, bool latestOnly)
{
    ...
    if (!string.IsNullOrWhiteSpace(transactionType))
    {
        ...
        var validFilterKeys = await _context.TransactionTypes
            .Where(tt => tt.Code == targetFilter || tt.TransactionTypeId.ToString() == targetFilter)
            .Select(tt => tt.TransactionTypeId.ToString())
            .ToListAsync();
    ...
    var projectedTransactions = await query
        .Distinct()
        .Select(t => new {
    ...
    if (latestOnly)
    {
        projectedTransactions = projectedTransactions
            .Where(t => t.TransactionDate.HasValue) // Explicit check
            .OrderByDescending(t => t.TransactionDate ?? DateTime.MinValue) // Safe fallback
            .GroupBy(t => t.CustomerAccountId)
            .Select(g => g.First())
            .ToList();
    }

    var activeBranchIds = projectedTransactions.Where(t => t.BranchId != null).Select(t => t.BranchId).Distinct().ToList();
    ...
    var activeTxnTypes = projectedTransactions.Where(t => !string.IsNullOrWhiteSpace(t.TransactionType)).Select(t => t.TransactionType.Trim()).Distinct().ToList();
    ...
    var currentAccountIds = projectedTransactions.Where(t => t.CustomerAccountId.HasValue).Select(t => t.CustomerAccountId!.Value).Distinct().ToList();
    var transactionRefNumbers = projectedTransactions.Select(t => t.TransactionReferenceNumber).Where(r => !string.IsNullOrEmpty(r)).Distinct().ToList();
    ...
    var transactionIds = projectedTransactions.Select(t => t.CustomerTransactionId).ToList();
    ...
    foreach (var txn in projectedTransactions)
    {
